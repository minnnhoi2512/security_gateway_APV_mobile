import { View, Text, Image, SafeAreaView, Pressable, TouchableOpacity, ScrollView } from 'react-native';
...
const notification = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = useGetUserNotificationQuery(userId ? { userID : userId } : { userID: "" }, {
    skip: !userId,
  });
      ...
      <View className="flex-row space-x-3">
        ...
        <View className="flex-1 space-y-1">
          <View className="flex-row flex-wrap">
            ...
            <Text className="text-gray-500 mx-1">{notification.notification.action}</Text>
            {/* {notification.target && (
          ...
          <Text className="text-sm text-gray-400">{notification.notification.sentDate.toString().split("T")[0]}</Text>
        </View>
