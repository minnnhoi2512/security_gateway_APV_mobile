...
const ListVisit: React.FC = () => {
  ...
  console.log("CCCD cua list vss: ", credentialCardId);
  2;
  // let data: Visit[] = [];
  ...
  // console.log("Data visit ne con:", visitOfUser);
  ...
  const renderVisit = ({ item }: { item: Visit }) => (
    ...
    >
      <Text className="text-lg font-semibold mb-1 text-white">
        Tên chuyến thăm: {item.visit.visitName}
      </Text>
      <Text className="text-sm text-white mb-1">
        Loại lịch trình: {item.visit.scheduleTypeName}
      </Text>
      <Text className="text-sm mb-1 text-white ">
        Khách thăm: {item.visitor.visitorName}
      </Text>
      <Text className="text-sm mb-1 text-white">
        Công ty: {item.visitor.companyName}
      </Text>
      <Text className="text-sm mb-1 text-white">
        Số điện thoại: {item.visitor.phoneNumber}
      </Text>
      <Text className="text-sm mb-1 text-white">
        Giờ bắt đầu dự kiến: {item.expectedStartHour}
      </Text>
      <Text className="text-sm text-white">
        Giờ kết thúc dự kiến: {item.expectedEndHour}
      </Text>
    </TouchableOpacity>
  );
